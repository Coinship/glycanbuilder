...
public class GlycanDocument extends BaseDocument implements SAXUtils.SAXWriter {
	...
	public Collection<javax.swing.filechooser.FileFilter> getFileFormats() {
		...
		filters.add(new ExtensionFileFilter("gws",
	...
	public void orderStructures(boolean descending) {
		...
		if (descending)
			sorted_structures = new TreeMap<Double, Vector<Glycan>>(
		...
		if (isEmpty() || current == null || current.isReducingEnd()) {
			...
			for (Residue b : brackets) {
				for (Linkage link : b.getChildrenLinkages())
					cur_structure.addAntenna(link.getChildResidue(),
							link.getBonds());
			}
		}
		return true;
	 ...
	 * @see #extractView
	 */
	public void copyResidues(Residue current, Vector<Residue> linked,
	 ...
	 * @see #extractView
	 */
	public void moveResidues(Residue current, Vector<Residue> linked,
		...
		for (int i = 0; i < structures.size(); i++) {
			...
			if (structure.removeResidue(toremove)) {
				if (structure.isEmpty())
					...
					Vector<Glycan> new_structures = structure
							.splitMultipleRoots();
					for (Iterator<Glycan> l = new_structures.iterator(); l
							.hasNext();)
	 ...
	 */
	public Collection<Glycan> parseString(String str, GlycanParser parser)
			throws Exception {

		Vector<Glycan> parsed = new Vector<Glycan>();
		if (parser instanceof GWSParser) {
			for (String t : TextUtils.tokenize(str, ";"))
				parsed.add(parser.readGlycan(t,
						theWorkspace.getDefaultMassOptions()));
		} else
			parsed.add(parser.readGlycan(str,
					theWorkspace.getDefaultMassOptions()));
	 ...
	 * @see GWSParser
	 */
	static public Collection<Glycan> parseString(String str, MassOptions opt)
			throws Exception {
		// parse structures
		Vector<Glycan> parsed = new Vector<Glycan>();
		for (String t : TextUtils.tokenize(str, ";"))
	...
	public void fromXML(Node struct_node, boolean merge) throws Exception {
		...
		for (Node g_node : g_nodes)
			structures.add(Glycan.fromXML(g_node,
					theWorkspace.getDefaultMassOptions()));
	}
