 ...
 * {@link ResiduePlacementDictionary}. The classes {@link ResidueRenderer} and
 * {@link LinkageRenderer} are used to draw the different parts of the
...
public class GlycanRenderer {
	 ...
	 */
	public void paint(Graphics2D g2d, Glycan structure,
		...
		paint(g2d, structure, selected_residues, selected_linkages, null,
	...
	public void paint(Graphics2D g2d, Glycan structure,
		...
		if (structure.isComposition())
			paintComposition(g2d, structure.getRoot(), structure.getBracket(),
					selected_residues, posManager, bboxManager);
		else {
			paintResidue(g2d, structure.getRoot(show_redend),
					selected_residues, selected_linkages, active_residues,
					posManager, bboxManager);
			paintBracket(g2d, structure.getBracket(), selected_residues,
	...
	protected void displayMass(Graphics2D g2d, Glycan structure,
	...
	private void paintComposition(Graphics2D g2d, Residue root,
	...
	private void paintResidue(Graphics2D g2d, Residue node,
			HashSet<Residue> selected_residues,
			HashSet<Linkage> selected_linkages,
		...
		for (Linkage link : node.getChildrenLinkages()) {
			...
			if (child_bbox != null && !posManager.isOnBorder(child)) {
				...
				theLinkageRenderer.paintEdge(g2d, link, selected, node_bbox,
		...
		boolean active = (active_residues == null || active_residues
				.contains(node));
		theResidueRenderer.paint(g2d, node, selected, active,
				posManager.isOnBorder(node), parent_bbox, node_bbox,
				support_bbox, posManager.getOrientation(node));

		// paint children
		for (Linkage link : node.getChildrenLinkages())
			paintResidue(g2d, link.getChildResidue(), selected_residues,
					selected_linkages, active_residues, posManager, bboxManager);
		...
		for (Linkage link : node.getChildrenLinkages()) {
			...
			if (child_bbox != null && !posManager.isOnBorder(child))
				theLinkageRenderer.paintInfo(g2d, link, node_bbox, border_bbox,
						child_bbox, child_border_bbox);
		}

	}

	private void paintBracket(Graphics2D g2d, Residue bracket,
			HashSet<Residue> selected_residues,
			HashSet<Linkage> selected_linkages,
			Collection<Residue> active_residues, PositionManager posManager,
			BBoxManager bboxManager) {
		...
		boolean active = (active_residues == null || active_residues
				.contains(bracket));
		theResidueRenderer.paint(g2d, bracket, selected, active, false,
			...
			if (child_bbox != null) {
				...
				if (!posManager.isOnBorder(child)) {
					...
					active = (active_residues == null || (active_residues
							...
							.contains(child)));
					theLinkageRenderer
							.paintEdge(g2d, link, selected, node_bbox,
									node_bbox, child_bbox, child_border_bbox);
				}

				// paint child
				paintResidue(g2d, child, selected_residues, selected_linkages,
						active_residues, posManager, bboxManager);

				// paint info
				if (!posManager.isOnBorder(child))
					theLinkageRenderer.paintInfo(g2d, link, node_bbox,
							node_bbox, child_bbox, child_border_bbox);
	...
	protected void paintQuantity(Graphics2D g2d, Residue antenna, int quantity,
			BBoxManager bboxManager) {
		...
		try {
			...
			if (!structure.isEmpty()) {
				...
				if (structure.isComposition()) {
					...
					// compute bounding boxes
					computeBoundingBoxesComposition(root, bracket, posManager,
							bboxManager);
				} else {
					...
					// assign positions
					posManager.add(root, new ResAngle(), orientation, false,
							true);
					assignPosition(root, false, orientation, root, posManager);

					posManager.add(bracket, new ResAngle(), orientation, false,
							true);
					assignPosition(bracket, false, orientation, bracket,
							posManager);
	...
	static private String makeCompositionText(Residue root, Residue bracket,
	...
	private void computeBoundingBoxesLR(Residue node,
		...
		Vector<Residue> region_m90b = posManager.getChildrenAtPosition(node,
				new ResAngle(-90), true);
		for (int i = 0; i < region_m90b.size(); i++) {
			computeBoundingBoxes(region_m90b.elementAt(i), posManager,
					bboxManager);
			if (i > 0)
		...
		Vector<Residue> region_p90b = posManager.getChildrenAtPosition(node,
				new ResAngle(90), true);
		for (int i = 0; i < region_p90b.size(); i++) {
			computeBoundingBoxes(region_p90b.elementAt(i), posManager,
					bboxManager);
			if (i > 0)
		...
		// position -90 (top)
		Vector<Residue> region_m90 = posManager.getChildrenAtPosition(node,
				new ResAngle(-90), false);
		for (int i = 0; i < region_m90.size(); i++) {
			computeBoundingBoxes(region_m90.elementAt(i), posManager,
					bboxManager);
			if (i > 0)
		...
		// position +90 (bottom)
		Vector<Residue> region_p90 = posManager.getChildrenAtPosition(node,
				new ResAngle(90), false);
		for (int i = 0; i < region_p90.size(); i++) {
			computeBoundingBoxes(region_p90.elementAt(i), posManager,
					bboxManager);
			if (i > 0)
		...
		// position 0 (right)
		Vector<Residue> region_0 = posManager.getChildrenAtPosition(node,
				new ResAngle(0));
		...
		Vector<Residue> region_m45 = posManager.getChildrenAtPosition(node,
				new ResAngle(-45));
		for (int i = 0; i < region_m45.size(); i++) {
			computeBoundingBoxes(region_m45.elementAt(i), posManager,
					bboxManager);
			if (i > 0)
		...
		Vector<Residue> region_p45 = posManager.getChildrenAtPosition(node,
				new ResAngle(45));
		for (int i = 0; i < region_p45.size(); i++) {
			computeBoundingBoxes(region_p45.elementAt(i), posManager,
					bboxManager);
			if (i > 0)
	...
	private void computeBoundingBoxesRL(Residue node,
		...
		// align position 0,45,-45 with node to not clash with 90b,-90b
		if (region_0.size() > 0) {
			bboxManager.alignRightsOnBottom(region_0, region_m45,
					theGraphicOptions.NODE_SPACE);
	...
	private void computeBoundingBoxesBracketLR(Residue bracket, Residue root,
		...
		for (int i = 0; i < bracket.getNoChildren(); i++) {
			...
			if (value == null) {
				...
				if (antennae.size() > 0)
					bboxManager.alignLeftsOnBottom(antennae.lastElement(),
	...
	private void computeBoundingBoxesBracketRL(Residue bracket, Residue root,
		...
		for (int i = 0; i < bracket.getNoChildren(); i++) {
			...
			if (value == null) {
				...
				if (antennae.size() > 0)
					bboxManager.alignRightsOnTop(antennae.lastElement(),
							antenna, theGraphicOptions.NODE_SPACE);
	...
	private void computeBoundingBoxesBracketTB(Residue bracket, Residue root,
		...
		for (int i = 0; i < bracket.getNoChildren(); i++) {
			...
			if (value == null) {
				...
				if (antennae.size() > 0)
					bboxManager.alignBottomsOnLeft(antennae.lastElement(),
							antenna, theGraphicOptions.NODE_SPACE);
	...
	private void computeBoundingBoxesBracketBT(Residue bracket, Residue root,
		...
		for (int i = 0; i < bracket.getNoChildren(); i++) {
			...
			if (value == null) {
				...
				if (antennae.size() > 0)
					bboxManager.alignTopsOnRight(antennae.lastElement(),
							antenna, theGraphicOptions.NODE_SPACE);
	...
	public BufferedImage getImage(Collection<Glycan> structures,
		...
		BufferedImage img = GraphicUtils.createCompatibleImage(d.width,
				d.height, opaque);
		...
		for (Glycan s : structures)
			paint(g2d, s, null, null, show_masses, show_redend, posManager,
					bboxManager);
