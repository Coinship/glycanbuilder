...
public class GlycoCTParser implements GlycanParser {
	...
	public Sugar toSugar(Glycan structure, BBoxManager manager) throws Exception {
		...
		if (structure.getBracket() != null) {
			ArrayList<GlycoNode> parents = (ArrayList<GlycoNode>) sugar
					.getNodes().clone();
	...
	static private Residue fromSugar(GlycoNode nm_current,
		...
		if (nm_current instanceof SugarUnitRepeat) {
			ret = fromSugarUnitRepeat((SugarUnitRepeat) nm_current, converter,
					tolerate_unknown_residues);
			...
			Residue current = fromIupacName(iupac_name, converter,
					converter == null);
			if (current == null) {
				...
				// parse again
				current = fromIupacName(iupac_name, converter,
						tolerate_unknown_residues);
				if (current == null)
					throw new Exception("Unrecognized residue type: "
							+ iupac_name + " " + tolerate_unknown_residues);

				// add substitutions
				addModifications(current, subs, converter,
						tolerate_unknown_residues);
			}
